# Configuração do ModMenu e YACL

## Visão geral

O Stack Size Adjuster oferece uma tela de configuração opcional para o cliente usando **ModMenu** e **YetAnotherConfigLib (YACL v3)** através de reflexão (`GuiHelper.getOptionalYaclFactory`), prevenindo falhas em servidores dedicados sem YACL.

---

## ⚙️ Especificação do arquivo de configuração

* **Caminho**: `config/stack-size-adjuster.json`
* **Formato**: JSON com rastreamento de versão de esquema.

### Esquema JSON e valores padrão:
```json
{
  "configVersion": 1,
  "items64Limit": 128,
...
## 🖥️ Arquitetura da tela YACL 3.9.5

A tela de configuração no cliente é construída via reflexão em `YaclScreenHelper`:
...
### Aviso de segurança
Ao inserir um valor superior a $39.768.215$, o YACL exibe um aviso dinâmico alertando sobre os riscos de transbordamento de inteiros em baús duplos.
