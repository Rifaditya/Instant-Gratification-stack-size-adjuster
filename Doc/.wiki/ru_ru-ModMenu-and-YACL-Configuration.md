# Конфигурация ModMenu и YACL

## Обзор

Stack Size Adjuster предоставляет опциональный графический интерфейс настроек через **ModMenu** и **YetAnotherConfigLib (YACL v3)** с использованием рефлексии (`GuiHelper.getOptionalYaclFactory`), что предотвращает сбои загрузчика классов на выделенных серверах без YACL.

---

## ⚙️ Спецификация файла конфигурации

* **Путь**: `config/stack-size-adjuster.json`
* **Формат**: JSON с отслеживанием версии схемы.

### Схема JSON и значения по умолчанию:
```json
{
  "configVersion": 1,
  "items64Limit": 128,
...
## 🖥️ Архитектура экрана YACL 3.9.5

Экран настроек клиента создается через рефлексию в `YaclScreenHelper`:
...
При вводе числа более $39,768,215$ YACL отображает всплывающее предупреждение о риске целочисленного переполнения в больших сундуках.
